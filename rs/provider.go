...
func Provider() terraform.ResourceProvider {
	return &schema.Provider{
		...
		ResourcesMap: map[string]*schema.Resource{
			...
			"rs_cm_network_gateway":     resourceCMNetworkGateway(),
			"rs_cm_route_table":         resourceCMRouteTable(),
			"rs_cm_security_group":      resourceCMSecurityGroup(),
			"rs_cm_security_group_rule": resourceCMSecurityGroupRule(),
			"rs_cm_server":              resourceCMServer(),
			"rs_cm_server_array":        resourceCMServerArray(),
			"rs_cm_ssh_key":             resourceCMSSHKey(),
			"rs_cm_subnet":              resourceCMSubnet(),
			"rs_cwf_process":            resourceCWFProcess(),
		},
